    ...
    handler: async (req, h) => {
        ...
        try {
            ...
            if (user) {
                ...
                const percentage_down_payment = purchase_price / user_down_payment;
                ...
                const eligible = await db.query(
                    `SELECT * FROM offers WHERE down_payment<=${percentage_down_payment} AND required_salary<=${user_salary} AND mortgage_term_length>=${user_mortgage_term_length}`
                );
                const not_eligible = await db.query(
                    `SELECT * FROM offers WHERE down_payment>${percentage_down_payment} OR required_salary>${user_salary} OR mortgage_term_length<${user_mortgage_term_length}`
                )
                return {
